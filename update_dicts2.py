...
const DIMS = {
  genre:{ label:'Genre / Media', zh:'媒體與風格', color:'#5b9fff', tokens:{
    ...
    'splice|edm|music|電子|節拍|音樂|合成器|素材|舞曲':{ en:['EDM','synth','musical','beat','splice material','loop','bass','techno'], zh:['電子舞曲','合成器','音樂性質','節拍','套裝素材','無縫循環','重低音','Techno電音'] },
  ...
  emotion:{ label:'Emotion / Vibe', zh:'情緒 / 氛圍', color:'#ffb347', tokens:{
    ...
    'positive|happy|win|正向|開心|愉悅|成功|肯定|贏':{ en:['positive','happy','cheerful','success','confirm','accept','reward','win','gain'], zh:['正向情緒','開心的','愉快的','成功完成','確認','接受','獎勵感','獲勝贏得','獲得'] },
    'negative|sad|fail|負面|悲傷|失敗|拒絕|錯誤':{ en:['negative','sad','fail','error','deny','wrong','wah-wah','decline'], zh:['負面情緒','悲傷的','失敗感','錯誤發生','拒絕發生','不對的聲音','失敗滑音','婉拒'] }
  }},
  material:{ label:'Material', zh:'材質', color:'#ffb347', tokens:{
    ...
    'bone|骨頭|骨骼|關節|骨折':{ en:['bone','crack bone','crunch','snap','skeletal','joint'], zh:['骨頭','斷骨聲','咀嚼/碎裂聲','清脆折斷','骨骼的','關節處'] },
    'flesh|meat|肉|血肉|人體|內臟|血|噴血|肉塊':{ en:['flesh','meat','blood','splatter','squelch','guts','visceral'], zh:['血肉','肉類','血水','噴濺濺出','黏稠擠壓聲','內臟','直覺/內臟的'] },
    'rope|chain|繩|鐵鍊|鍊子|鐵絲|纜繩':{ en:['rope','chain','wire','cable','rattle chain'], zh:['繩子','鐵鍊','鋼絲鐵絲','纜線','鐵鍊碰撞聲'] },
  }},
  action:{ label:'Action', zh:'動作', color:'#f06090', tokens:{
    ...
    'roll|滾|翻滾|滾動|旋轉|滾落|轉動':{ en:['roll','tumble','spin','rotate','barrel roll','rolling'], zh:['滾動','翻滾','快速旋轉','轉動','桶狀翻滾','持續滾動'] },
    'fly|whoosh|飛|掠過|穿過|飛行|飛翔|劃過|閃過':{ en:['whoosh','fly','pass','sweep','zip','dart','rush','soar','swish'], zh:['呼嘯而過','飛行','掠過經過','掃過','快速拉開','如飛鏢般穿過','衝刺而過','翱翔高飛','揮動物品'] },
    ...
    'crash|衝撞|碰撞|撞車|墜機|撞擊|相撞':{ en:['crash','collision','impact','wreck','smash','crunch','vehicle crash'], zh:['墜毀/撞毀','碰撞','衝擊打擊','殘骸/失事','用力砸碎','粉碎聲響','車輛撞擊'] },
    ...
    'knock|敲門|叩|敲擊|叩門|敲打門':{ en:['knock','rap','tap','bang door','door knock'], zh:['敲擊','快速敲聲','輕敲','用力拍門','敲門聲'] },
    ...
    'eat|drink|咬|嚼|喝|吞|咀嚼|飲食':{ en:['bite','chew','drink','swallow','eat','munch','slurp'], zh:['咬一口','咀嚼','喝水','吞嚥','吃東西','用力咀嚼','吸溜喝湯'] },
    'charge|riser|rise|build|蓄力|聚能|爬升|集氣|充能':{ en:['charge','charge up','riser','rise','build-up','power up','crescendo','swell'], zh:['蓄力','充能完畢','情緒爬升','升起','氣氛鋪陳','威力提升','漸強音','聲勢漸漲'] },
    'collect|gain|get|收集|獲得|取得|得到|撿起':{ en:['collect','gain','get','acquire','pickup','obtain','receive'], zh:['收集','獲得收益','取得','獲得/擁有','撿起','獲取','收到/接收'] },
  }},
  weapon:{ label:'Weapon Type', zh:'武器類型', color:'#ff6b5b', tokens:{
    ...
    'blaster|sci-fi gun|雷射槍|光線槍|科幻武器|電漿槍':{ en:['blaster','sci-fi gun','plasma rifle','laser gun','stun gun','ray gun'], zh:['爆能槍','科幻槍枝','電漿步槍','雷射槍','電擊槍','光線槍'] },
  }},
  environment:{ label:'Environment', zh:'環境 / 空間', color:'#4dd9ac', tokens:{
    ...
    'cave|洞穴|回響|迴聲|隧道|殘響|空曠|回音|空洞|空間感':{ en:['cave','reverb','echo','cavern','tunnel','reverberant','large space','stone room'], zh:['洞穴','殘響效果','回音現象','大岩洞','隧道內','充滿回音的','廣大空間','石室'] },
    'forest|森林|樹林|叢林|自然|樹葉|樹枝|樹木|林中':{ en:['forest','jungle','nature','leaves','branch','birds','woodland','rustling leaves'], zh:['森林','叢林雨林','大自然','樹葉','樹木枝幹','鳥類','林地','樹葉沙沙聲'] },
    ...
    'outdoor|室外|戶外|開放|外面|野外|戶外空間':{ en:['outdoor','exterior','open air','field','meadow','open space'], zh:['戶外地帶','外部空間','露天空曠處','遼闊田野','草原','空地空間'] },
    'indoor|室內|房間|密閉|房子|屋內|室內空間':{ en:['indoor','interior','room','enclosed','inside','hallway','corridor'], zh:['室內','內部空間','房間內','密閉空間','在裡面','玄關大廳','走廊通道'] },
    'night|夜晚|夜間|深夜|夜裡|黑夜':{ en:['night','crickets','night ambience','nocturnal','night insects'], zh:['夜晚','蟋蟀蟲鳴','夜間氛圍','夜行性的','夜晚昆蟲'] },
    'space|太空|宇宙|外太空|星球':{ en:['space','sci-fi ambience','cosmic','vacuum','spacecraft'], zh:['太空','科幻環境音','宇宙的','真空狀態','太空船內部'] },
  }},
  ...
  mechanical:{ label:'Mechanical', zh:'機械 / 工業', color:'#ffb347', tokens:{
    ...
    'electric|elecetric|spark|zap|電流|電力|火花|觸電|靜電':{ en:['electric','electricity','elecetric','spark','zap','shock','static','arc','surge'], zh:['電子的','電力','通電','電火花','快速電擊','觸電休克','靜電干擾','電弧放電','電流湧浪'] },
  ...
  magic:{ label:'Magic / Sci-Fi', zh:'魔法 / 科幻', color:'#5b9fff', tokens:{
    'magic|spell|魔法|咒語|施法|附魔|奧術|神秘|魔力|法術':{ en:['magic','spell','cast','enchant','arcane','mystical','incantation','magic cast'], zh:['魔法','咒語','施放法術','附魔強化','奧術魔法','神秘超自然','吟唱咒語','魔法施展'] },
    ...
    'ice magic|冰系|冰凍|冰魔法|冰封|凍結':{ en:['ice magic','freeze','frost','ice spell','blizzard magic','ice crack'], zh:['冰系魔法','凍結','結霜','冰法術','暴風雪魔法','極冰碎裂'] },
    ...
    'shield|barrier|護盾|屏障|防護|防禦|結界':{ en:['shield','barrier','force field','deflect','magical shield','bubble shield'], zh:['護盾','屏障','力場防護','彈開攻擊','魔法護盾','泡泡防護罩'] },
    'laser|beam|雷射|光束|能量|射線|光砲':{ en:['laser','beam','energy','ray','plasma','charge up','laser shot'], zh:['雷射射線','光束','能量','射線','電漿','蓄力充滿體','雷射射擊'] },
    'shimmer|sparkle|shine|flash|光芒|閃爍|魔法粒子|閃耀|光效|粒子|閃現|發光':{ en:['shimmer','sparkle','twinkle','shine','flash','glow','glare','fairy','magical glow','particle'], zh:['微波閃爍','火光閃爍','星星閃爍','發光閃耀','閃現/閃光','發出光圈','刺眼強光','精靈音效','魔法光芒','魔法粒子'] },
    'dark magic|暗黑|黑魔法|詛咒|腐蝕|暗影|黑暗':{ en:['dark magic','curse','corruption','shadow','void','necromancy','dark energy'], zh:['黑魔法','詛咒','腐蝕魔法','暗影能量','虛空','死靈法術','黑暗能量'] },
    ...
    'heal|buff|治癒|補血|補魔|藥水|增益|狀態恢復':{ en:['heal','mana potion','health potion','buff','restore','resurrect','magic chime'], zh:['治癒回血','魔力藥水','生命藥水','增益狀態','恢復原狀','復活','魔法鐘聲'] },
  }},
  ui:{ label:'UI / Interface', zh:'介面音效', color:'#5b9fff', tokens:{
    ...
    'success|complete|win|成功|完成|勝利|解鎖|升級|獎勵|達成|任務|獲勝|贏':{ en:['success','complete','win','unlock','reward','level up','achievement','fanfare','triumph','quest complete'], zh:['成功','完成任務','贏得勝利','解鎖新要素','獲得獎勵','等級提升','解鎖成就','勝利號角','旗開得勝','任務完成'] },
    'error|fail|wrong|錯誤|失敗|拒絕|不對|無效|出錯':{ en:['error','fail','wrong','deny','buzz','decline','invalid','negative','error tone'], zh:['錯誤','失敗的聲音','錯了的音效','拒絕操作','錯誤蜂鳴','婉拒','無效操作','負面反饋','錯誤提示音'] },
    'click|tap|點擊|輕觸|按下|點選|按鍵|點一下':{ en:['click','tap','button press','select','UI click','mouse click','touch'], zh:['點擊聲','輕觸畫面','按下實體按鍵','選擇介面','介面點擊','滑鼠點擊','觸碰螢幕'] },
    ...
    'game|遊戲|音效|得分|扣血|命中|拾取|撿起|遊戲音|金幣|背包|裝備':{ en:['game sound','score','damage','hit marker','pickup','collect','game UI','coin','health','inventory','equip'], zh:['遊戲音效','加分','受到傷害','命中提示','撿起道具','收集物品','遊戲介面','獲得金幣','補充體力','打開背包','裝備武器'] },
    'gain|get|collect|獲得|取得|得到|收集|獲取':{ en:['gain','get','collect','acquire','receive','obtain','fetch'], zh:['獲得收益','取得','收集','獲取','收到','獲得','拿回來'] },
    'beep|chime|type|叮|嗶|鐘聲|提示音|嗶嗶|叮叮|打字聲':{ en:['beep','chime','ding','ping','tone','blip','bell','notification sound','typing','typewriter'], zh:['嗶嗶聲','風鈴鐘聲','叮的一聲','Ping聲響','單一聲調','遊戲電子閃爍音','鈴鐺聲','通知音效','鍵盤打字聲','復古打字機'] },
  }}
};"""
